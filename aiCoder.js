...
async function writeFile(filePath, content, makeBackup = false) {
...
async function appendFile(filePath, content, makeBackup = false) {
...
async function rollbackFile(pathToBackupFile) {
...
async function restoreFileFromBackup() {
  ...
  const allBackupFiles = await listFilesMatchingName(`${backupFolder}/${filePath}_backup_`);
...
async function listFilesMatchingName(baseFileName) {
  const results = [];
